...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://lill-e.onrender.com/api/v1/dalle', {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://lill-e.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e76] text-[16px] max-w-[500px]">
          Create imaginative and visually stunning images generate through
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
          ...
          <FormField
            ...
            type="text"
            name="prompt"
            placeholder="A hyper realistic beautiful bluenose pitbull and bulldog mix"
            value={form.prompt}
            handleChange={handleChange}
            isSurpriseMe
